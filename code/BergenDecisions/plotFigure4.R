...
plotFigure4 <- function(res, ifPdf=TRUE, fileName="figures/TotalDamageCostsAdaptation.pdf")
{
    ...
    plot(c(2016,2100), c(a,a), type="l", ylim=c(0,4.5), xlab="Year of adaptation measure",
         ylab="Total cost 2016-2100 (billion NOK)", main="")
    year <- (2016:2100)
    k <- 0.35
    for(i in 1:85)
    {
        ...
        polygon(c(a-k, a+k, a+k, a-k), c(c, c, b, b), col="#6594ED60", border="#6594ED60")
        d <- adapt.median[i]/1e3
        lines(c(a-k, a+k), c(d,d), lwd=3, col="#6594ED")
    }
    a <- median(cumsum.yearly.damage[,85])/1e3
    lines(c(2016-k,2100+k),c(a,a), col="black", lwd=2)
    a <- quantile(cumsum.yearly.damage[,85], 0.95)/1e3
    lines(c(2016-k,2100+k),c(a,a), col="black", lty=3, lwd=3)
