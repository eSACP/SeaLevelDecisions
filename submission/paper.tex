...
Table \ref{tab:stormsurge} contains the total projected storm surges for Esbjerg, corresponding to the 20-year and 100-year historical surges, with and without sea level rise. We see that the historical maximum 433 cm is very likely to be exceeded by 2100.
...
\begin{figure}[!hbpt]
\begin{center}
\includegraphics[width=0.9\linewidth]{Floodmaps_Harbour.png}
\caption{Flood extents and depths for the city of Esbjerg in year 2100 during storm surges with a 20 year return period (upper row) and a 100 year return period (lower row) with (a) no sea level rise and with sea level rise corresponding to RCP 85 (5th percentile (b), median (c) and 90th percentile (d). }
\label{fig:esbjerg}
\end{center}
\end{figure}
...
A simplistic analysis of projected damage costs at the year 2100, not taking into account the uncertainty, would use  the median  historical damage cost multiplied by  the median damage effect factor at 2100 at the median sea level rise projected for 2100. This yields a damage cost of NOK 335 million (the grey vertical line in Figure \ref{fig:costunc}. Similar results obtain when allowing sea level or effect factor to vary, holding the other quantities at the median (yellow and purple dots on top of Figure \ref{fig:costunc}). However, allowing only the damage cost to vary yields a median cost of 3.84 billion (green dot on top of Figure \ref{fig:costunc}). The appropriate uncertainty analysis for our model should draw each of sea level, effect factor and damage cost at random from their 2100 distributions. This corresponds to a median cost of 3.00 billion NOK, a factor of 9 higher than the simplistic value. Over 99\% of the costs in our model are higher than the simplistic median.
...
Important to continue an iterative and interactive with end-users, as the decision framework provides actionable information to decision-makers, who will then make their own decisions. These decision are then incorporated into the current adaptation strategy. Continued loop to identify potential vulnerabilities of the approaches across a wide-range of possible futures.

Take home message

\begin{itemize}
\item It is possible to account for uncertainty even if you do a light touch decision-making approach -- Explain what it implies if you don't account and if you do
\item Proof of concept -- connect in a probabilistic way (normally done in a discrete way) -- consider uncertainty in both ways
\item Improve the accessibility and demonstrating the general applicability of policy first approaches -- towards more generic toolkit
\end{itemize}
